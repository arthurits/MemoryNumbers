...
public partial class Board : PictureBox
{
    ...
    {
        if (pctCorrect is not null && pctWrong is not null && _svgCorrect is not null && _svgWrong is not null)
        {
    ...
    private async void ButtonClicked(object? sender, RoundButton.ButtonClickEventArgs e)
    {
        ...
        // Find the reason why this is necessary, otherwise the last button clicked is not rendered on time
        await Task.Delay(1);
